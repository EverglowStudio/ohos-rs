...
fn gen_tracing_debug(js_name: &str, parent_js_name: Option<&String>) -> TokenStream {
  ...
  quote! {
    napi_ohos::bindgen_prelude::tracing::debug!(target: "napi", "{}", #full_name);
  }
}
  ...
  fn try_to_tokens(&self, tokens: &mut TokenStream) -> BindgenResult<()> {
    ...
    if self.module_exports {
      (quote! {
        ...
        unsafe extern "C" fn #intermediate_ident(
          ...
          _napi_module_exports_: napi_ohos::bindgen_prelude::sys::napi_value,
      ...
      && self.pre_call.is_empty()
    {
      ...
      quote! {
        ...
        if napi_ohos::__private::___CALL_FROM_FACTORY.with(|inner| inner.get()) {
            #return_from_factory
        }
        #function_call_inner
      }
        ...
        NapiFnArgKind::PatType(path) => {
          if &path.ty.to_token_stream().to_string() == "Env" {
            ...
            if let syn::Type::Path(path) = path.ty.as_ref() {
              // get `Reference` in `napi_ohos::bindgen_prelude::Reference`
              if let Some(p) = path.path.segments.last() {
                if p.ident == "Reference" {
                      ...
                      }) = generic_type
                      {
                        if let Some(syn::PathSegment { ident, .. }) = segments.first() {
                          ...
                          args.push(
                            quote! {
                              {
                                <#ident as napi_ohos::bindgen_prelude::FromNapiValue>::from_napi_value(env, cb.this)?.into()
                              }
                            },
                          );
                          skipped_arg_count += 1;
                        ...
                        }) = elem.as_ref()
                        {
                          if let Some(syn::PathSegment { ident, .. }) = segments.first() {
                            ...
                            let token = if mutability.is_some() {
                              ...
                              quote! { <#ident as napi_ohos::bindgen_prelude::FromNapiMutRef>::from_napi_mut_ref(env, cb.this)?.into() }
                            } else {
                              quote! { <#ident as napi_ohos::bindgen_prelude::FromNapiRef>::from_napi_ref(env, cb.this)?.into() }
                            };
                            args.push(token);
                            skipped_arg_count += 1;
                            continue;
                  ...
                  refs.push(make_ref(quote! { cb.this }));
                  args.push(quote! { <napi_ohos::bindgen_prelude::This as napi_ohos::bindgen_prelude::FromNapiValue>::from_napi_value(env, cb.this)? });
                  skipped_arg_count += 1;
                  continue;
                }
              }
  ...
  fn gen_cb_arg_conversion(
    ...
    for (i, ty) in cb.args.iter().enumerate() {
      ...
      arg_conversions.push(
        quote! { <#maybe_has_lifetime_ty as napi_ohos::bindgen_prelude::ToNapiValue>::to_napi_value(env, #cb_arg_ident)? },
      );
    }
  ...
  fn gen_fn_return(&self, ret: &Ident) -> BindgenResult<TokenStream> {
    ...
    if let Some(ty) = &self.ret {
      ...
      if self.kind == FnKind::Constructor {
        ...
        Ok(quote! {
          <#return_ty as napi_ohos::bindgen_prelude::ToNapiValue>::to_napi_value(env, #ret)
        })
      }
    } else {
      Ok(quote! {
    ...
    if self.parent.is_some() || cfg!(test) {
      ...
      if self.module_exports {
        return quote! {
          ...
          #[allow(non_snake_case)]
          #[allow(clippy::all)]
          unsafe fn #cb_name(env: napi_ohos::bindgen_prelude::sys::napi_env, exports: napi_ohos::bindgen_prelude::sys::napi_value) -> napi_ohos::bindgen_prelude::Result<napi_ohos::bindgen_prelude::sys::napi_value> {
          ...
          napi_ohos::ctor::declarative::ctor! {
            ...
            fn #module_register_name() {
              napi_ohos::bindgen_prelude::register_module_export_hook(#cb_name);
            }
          }
